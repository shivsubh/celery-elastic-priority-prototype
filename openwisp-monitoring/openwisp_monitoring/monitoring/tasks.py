...
@gevent_safe(reason="Fast TSDB API writes via networking")
...
@gevent_safe(reason="Fast TSDB API batched writes via networking")
...
@blocking(reason="Time-consuming HTTP operations")
def delete_timeseries(key, tags):
    timeseries_db.delete_series(key=key, tags=tags)


@shared_task
@blocking(reason="Database schema migration scripts")
